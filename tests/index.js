    ...
    it('HTML file contains table of contents', done => {
      execa('./bin/index.js', ['./tests/test.md', './test-toc-output.pdf', '--auto-toc', '--debug'])
        .then(result => {
          ...
          htmlContent.includes('<pre><code class="hljs bash language-bash"><span class="hljs-comment"># a comment in a shell script</span>').should.be.true();
          htmlContent.includes('<pre><code class="hljs"><span class="hljs-comment"># another comment in a shell script</span>').should.be.true();
          done();
        })
    });
  });
